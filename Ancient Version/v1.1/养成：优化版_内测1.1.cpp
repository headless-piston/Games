...
int ch1, oklog;
int l, yes;
int user[6], num, logging, need[11]={20,80,100,300,500,500,500,1000,2000,5000}, lxp[10]={20,100,200,500,1000,1500,2000,3000,5000,10000};
int choose, choose1, choose2;
int change1, change2;
int answer;
long long hp0[4], xp0[4], lv0[4], cd0[4], hup[4], hp, xp, lv, cd, hg, xp1, up;
bool find;
char p[4][6], pc;
char tryp, trypc;
...
void candy() {
	if (cd0[logging] > 0) {
		...
		if (random % 10 == 0 || random % 10 == 4) {
			color(green);
			print(56);
			printf("You get %d hp and %lld xp.",random % 20 + 10,random % 50 + 20 + lv0[logging] * 10);
			hp0[logging] += random % 20 + 10;
			xp0[logging] += random % 50 + 20 + lv0[logging] * 10;
		} else if (random % 10 == 1 || random % 10 == 2 || random % 10 == 3) {
			color(red);
			print(57);
			printf("You lost %d hp and %lld xp.", random % 10 + 3, random % 40 + 10 + lv0[logging] * 25);
			...
			if (random == 2048) {
				...
				if (answer == 1) {
					if (xp0[logging] > 5 * (random % 1000)) {
						color(green);
						printf("You pull that up successfully.The woman let a child give you some candy.\n");
						printf("You get %d xp and %d candy.\n", random % 50 + 10, random % 5 + 1);
						xp0[logging] += random % 50 + 10;
						cd0[logging] += random % 5 + 1;
					} else {
						color(red);
						printf("You can't pull that up either.You are unhappy.\n");
						printf("You lost %d xp.\n", random % 20 + 10);
						xp0[logging] -= random % 20 + 10;
					}
				} else if (answer == 0) {
...
void information() {
	...
	xp1=xp0[logging];
	for(int i=1;i<lv0[logging];i++){
		xp1-=need[i-1];
	}
	for(int i=1;i<=20;i++){
		if(i*5<=(xp1*100.0/need[lv0[logging]-1])){
			color(green);
			printf("*");
		}else{
			color(yellow);
